...
ダッシュボード内のノートも参考に。「30日間は長期的なサービスの健全性、7日間は短期的な傾向把握」。週次レポートの SLI / EBR は SLO Status API で対象週・先週の7日間をそれぞれ取得する。SLO 定義の 30d は長期目標として区別し、バーンレートが1を超えた区間を例示すると、読み手のトレース調査を助ける。
