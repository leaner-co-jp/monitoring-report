...
| `templates/report.md` | レポート本体（🔗 調査リンク / §1 総合評価 / §2 調査 / §3 付録）と運用ガイドのトグル | **生成前に全文** |
| `scripts/validate-report.py` | 完成成果物の構造を機械的に検査（必須見出し、未置換プレースホルダ、§2 と発火の整合、ガイド残存、台帳の校正メモ） | 書き込み前 |
...
1. 出力先 DB を確認 → 同一ウインドウのページの有無で分岐（新規 / 中止 / 更新）
2. テンプレートを全文読み取る（生成前の必須ゲート。抜粋で進めない）
...
9. `templates/report.md` を土台に合成して完成成果物を作る（手組みしない）
10. 書き込み前検証（`scripts/validate-report.py` ＋ `output.md` のチェック）
